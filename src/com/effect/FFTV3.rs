...
void custom_fft(uint32_t x, uint32_t y, int is_row) {
	...
	for (int n = 0; n < width; n++) {
		...
		if (is_row == 1) {
			...
			rsSetElementAt_float4(gOutRe, re, (x*width)+n);
			rsSetElementAt_float4(gOutIm, im, (x*width)+n);


			float4 rr = rsGetElementAt_float4(gOutRe, (x*width)+n);

		} else {

			rsSetElementAt_float4(gOutRe, re, y+(n*width));
...
void filter() {
	...
	rsForEach(gScript, gInRe, gInIm, &is_row, 0, &restrict_for);
...
void root(const uchar4 *v_in_re, uchar4 *v_in_im, const int *usrData,
	...
	if (is_row == 1) {
		...
		for (int i = 0; i < width; i++) {
			const uchar4 *elementRe = rsGetElementAt(gInRe, x, i);
			float4 color = (rsUnpackColor8888(*elementRe));



			rsSetElementAt_float4(re1d, color, i);

			uchar4 c = rsPackColorTo8888((float4) 0);

			rsSetElementAt_uchar4(gInIm, c, x, i);

			const uchar4 *elementIm = rsGetElementAt(gInIm, x, i);

			float4 imcolor = (rsUnpackColor8888(*elementIm));
		...
		for (int i = 0; i < height; i++) {
			...
			float4 color = rsGetElementAt_float4(gOutRe, y+(i*width));
			float4 imcolor = rsGetElementAt_float4(gOutIm, y+(i*width));
